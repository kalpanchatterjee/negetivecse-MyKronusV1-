...
public class TaskRepo {
	...
	public List<TaskModel> getTaskDetails(TaskModel tm) {
		...
		String text = (tm.getText() != null && tm.getText() != null && !tm.getText().trim().equals(""))
				? tm.getText().trim()
				: "";
		String typeofDate = tm.getTypeofDate() != null && !tm.getTypeofDate().equals("") ? tm.getTypeofDate()
				: "NofilterDate";
		...
		String localoffsetTimeZone = (tm.getLocaloffsetTimeZone() != null && !tm.getLocaloffsetTimeZone().equals(""))
				? tm.getLocaloffsetTimeZone()
				: "";
		...
		sql.append(
				" SELECT cmt.task_status, cmt.task_id, cmt.task_name, ifnull(cmt.priority_id,'') as priority_id,CONCAT(cu.user_first_name,' ',user_last_name) AS uname ,");

		sql.append("  IFNULL( DATE_FORMAT(CONVERT_TZ(CONCAT(cmt.task_start_date ,' ', cmt.task_start_time), '"
				+ serverTimeZone + "',  ifnull(cu.user_timezone, '" + localoffsetTimeZone
				+ "') ),'%h:%i %p'),'')AS 'startTime',");
		sql.append(
				"  IFNULL( DATE_FORMAT(CONVERT_TZ(CONCAT(cmt.task_end_date ,' ', cmt.task_end_time),'" + serverTimeZone
						+ "',  ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%h:%i %p'),'')AS 'endTime',");
		sql.append("  IFNULL( DATE_FORMAT(CONVERT_TZ(CONCAT(cmt.task_start_date ,' ', cmt.task_start_time), '"
				+ serverTimeZone + "', ifnull(cu.user_timezone, '" + localoffsetTimeZone
				+ "') ),'%b %d %Y'),'') AS 'startDate',");
		sql.append(
				"  IFNULL( DATE_FORMAT(CONVERT_TZ(CONCAT(cmt.task_end_date ,' ', cmt.task_end_time),'" + serverTimeZone
						+ "', ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%b %d %Y'),'') AS 'endDate',");
		sql.append("  IFNULL( DATE_FORMAT(CONVERT_TZ(cmt.task_due_by_date, '" + serverTimeZone
				+ "', ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%b %d %Y'),'-') AS 'taskDueBy',");
		sql.append(
				" DATE_FORMAT(IFNULL((IFNULL(cu.last_updated_timestamp, cu.created_timestamp)), CURRENT_TIMESTAMP),'%y%m%d%H%i%s%f') AS imgTimeStamp,");
		sql.append("  IFNULL( cmt.task_due_by_date, '-') AS 'taskDueDate',");
		sql.append(
				"  IFNULL(cmt.actual_time,'') AS actTime,ifnull(cmt.task_comment,'') as taskComment,IFNULL(cmt.elapsed_time,'') AS elapsed_time, cmt.created_by, cmt.task_billable, cmt.actual_time_modified, cmt.task_type, ");
		sql.append(
				"  IFNULL(det.totAct, '') as detActTime,cu.user_timezone,IFNULL(cu.user_timezone_intz,'IST') AS bbm_pin, ");
		sql.append(
				"  IFNULL(cmt.task_group,'') as group_id, IFNULL(cmg.group_code, '') as group_code, IFNULL(cmg.group_name, '') as group_name ");
		...
		sql.append(
				"  					FROM co_mykronus_tasks_details WHERE status='Y' group by task_id ) as det on  det.task_id =  cmt.task_id ");
		...
		if (!taskId.equals("")) {
			...
			if (typeofDate.equals("NofilterDate")) {
				...
				sql.append(" 				AND (DATE_FORMAT(CONVERT_TZ(cmtd.created_timestamp,'" + serverTimeZone
						+ "', ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%Y-%m-%d')='" + presentDate
						+ "'))");
				sql.append(" 		)");
			}
			if (typeofDate.equals("filterDate")) {
				sql.append(" AND ( ");
				sql.append(
						"   (cmtd.task_status NOT IN('Completed','Cancel') AND (DATE_FORMAT(CONVERT_TZ(cmtd.created_timestamp,'"
								+ serverTimeZone + "', ifnull(cu.user_timezone, '" + localoffsetTimeZone
								+ "') ),'%Y-%m-%d')='" + presentDate + "')) ");
				sql.append(" 	   OR ");
				sql.append("   (cmtd.task_status='Completed' AND (DATE_FORMAT(CONVERT_TZ(cmtd.created_timestamp,'"
						+ serverTimeZone + "',ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%Y-%m-%d')='"
						+ presentDate + "')) ");
				sql.append(" ) ");
			}
		}
		...
		if (sortValue != null && sortValue.equals("name")) {
			...
			sql.append(
					" ORDER BY IF(cmt.task_status='Completed',4,IF(cmt.task_status='Paused',3,IF(cmt.task_status='Created',1,2))) ASC");
		...
		List<TaskModel> getTaskDetails = jdbctm.query(sql.toString(),
				BeanPropertyRowMapper.newInstance(TaskModel.class));
		...
		for (TaskModel vtm : getTaskDetails) {
			...
			if (!vtm.getTask_status().equals("Completed")) {
				...
				sql.append("	date_format(convert_tz('" + Utilcollection.getTimeStamp() + "','" + serverTimeZone
						+ "','" + (vtm.getUser_timezone() == null ? localoffsetTimeZone : vtm.getUser_timezone())
						+ "'), '%Y-%m-%d %H:%i:%s') ");
				sql.append("   , ");
				sql.append("  (SELECT date_format(convert_tz(MIN(created_timestamp), '" + serverTimeZone + "', '"
						+ (vtm.getUser_timezone() == null ? localoffsetTimeZone : vtm.getUser_timezone())
						+ "'), '%Y-%m-%d %H:%i:%s') from co_mykronus_tasks_details where task_id=" + vtm.getTask_id()
						+ " AND task_status='Inprogress' ) ");
				sql.append(" ),'') as elapsed_time ");
				List<TaskModel> getelapsed_time = jdbctm.query(sql.toString(),
						BeanPropertyRowMapper.newInstance(TaskModel.class));
				for (TaskModel v : getelapsed_time) {
					vtm.setElapsed_time(v.getElapsed_time().substring(0,
							v.getElapsed_time().lastIndexOf(".") > 0 ? v.getElapsed_time().lastIndexOf(".")
									: v.getElapsed_time().length()));
	...
	public List<TaskModel> createTask(TaskModel tm) {
		...
		try {
			sql.append(
					"INSERT INTO co_mykronus_tasks (task_name,task_status,company_id,created_by,created_date,created_timestamp,task_due_by_date, task_group)");
			sql.append("VALUES(?,?,?,?,?,?,?,?)");
			...
			if (ctr != 0) {
				...
				sql1.append(
						"INSERT INTO co_mykronus_tasks_details (task_id,task_status,company_id,created_by,created_date,last_updated_date,last_updated_timestamp,last_updated_by,created_timestamp) ");
				sql1.append("VALUES(?,?,?,?,?,?,?,?,?)");
				ctr = jdbctm.update(connection -> {
	...
	public List<TaskModel> updateTask(TaskModel tm) {
		...
		try {
			...
			if (status.equals("Paused")) {
				String query = "SELECT MAX(created_timestamp) AS Tasktimestamp FROM co_mykronus_tasks_details  WHERE task_id="
						+ tm.getTask_id();
				String taskTime = jdbctm.queryForObject(query, String.class);

				query = " SELECT CONCAT('', TIMEDIFF( date_format('" + Utilcollection.getTimeStamp()
						+ "', '%Y-%m-%d %H:%i:%s'), date_format('" + taskTime
						+ "', '%Y-%m-%d %H:%i:%s') )) as act_time ";
				act_time = jdbctm.queryForObject(query, String.class);

				query = "SELECT task_detail_id FROM co_mykronus_tasks_details  WHERE task_id=" + tm.getTask_id()
						+ " ORDER BY task_detail_id DESC LIMIT 1 ";
				String latestDetailId = jdbctm.queryForObject(query, String.class);

				String sqlInsert1 = "   UPDATE co_mykronus_tasks_details SET actual_time= '" + act_time
						+ "', task_status_stop_timestamp=  '" + Utilcollection.getTimeStamp() + "', last_updated_by="
						+ userId + "  WHERE task_detail_id=" + latestDetailId;
				jdbctm.update(sqlInsert1);
			}
			if (status.equals("Completed")) {
				String query = "SELECT task_status FROM co_mykronus_tasks_details  WHERE task_id=" + tm.getTask_id()
						+ " ORDER BY task_detail_id DESC LIMIT 1 ";
				String status_prev = jdbctm.queryForObject(query, String.class);
				if (!status_prev.equals("Paused") && !status_prev.equals("Created")) {
					query = "SELECT MAX(created_timestamp) AS Tasktimestamp FROM co_mykronus_tasks_details  WHERE task_id="
							+ tm.getTask_id();
					String max_timestamp = jdbctm.queryForObject(query, String.class);

					query = " SELECT CONCAT('',TIMEDIFF( date_format('" + Utilcollection.getTimeStamp()
							+ "', '%Y-%m-%d %H:%i:%s') , date_format('" + max_timestamp
							+ "', '%Y-%m-%d %H:%i:%s') )) as act_time ";
					act_time = jdbctm.queryForObject(query, String.class);
					...
					String sqlInsert1 = "   UPDATE co_mykronus_tasks_details SET actual_time= '" + act_time
							+ "' , task_status_stop_timestamp=  '" + Utilcollection.getTimeStamp()
							+ "', last_updated_by=" + userId + "    WHERE task_detail_id=" + latestDetailId;
					jdbctm.update(sqlInsert1);
				} else {
					query = "SELECT task_detail_id FROM co_mykronus_tasks_details  WHERE task_id=" + tm.getTask_id()
							+ " ORDER BY task_detail_id DESC LIMIT 1 ";
					...
					String sqlInsert1 = "   UPDATE co_mykronus_tasks_details SET  task_status_stop_timestamp=  '"
							+ Utilcollection.getTimeStamp() + "', last_updated_by=" + userId
							+ "    WHERE task_detail_id=" + latestDetailId;
					jdbctm.update(sqlInsert1);
				}
			}
			...
			if (!status.equals("")) {
				...
				sql1.append("  VALUES(?,?,?,?,?,? )");
				int ctr = 0;
				ctr = jdbctm.update(sql1.toString(), tm.getTask_id(), status, companyId, userId,
						Utilcollection.getDate(), Utilcollection.getTimeStamp());
				if (status.equals("Completed")) {
					sql1 = new StringBuilder();
					sql1.append(" SELECT CONCAT('', TIMEDIFF( ");
					sql1.append(" 	 (SELECT MAX(created_timestamp) from co_mykronus_tasks_details where task_id="
							+ tm.getTask_id() + " AND task_status='Completed' ) ");
					sql1.append("   , ");
					sql1.append("  (SELECT MIN(created_timestamp) from co_mykronus_tasks_details where task_id="
							+ tm.getTask_id() + " AND task_status='Inprogress' ) ");
					sql1.append(" ) ) ");
					elapsed_time = jdbctm.queryForObject(sql1.toString(), String.class);
					System.out.println(sql1.toString());
					String sqlInsert1 = "   UPDATE co_mykronus_tasks SET elapsed_time= '" + elapsed_time
							+ "' WHERE task_id=" + tm.getTask_id();
					jdbctm.update(sqlInsert1);
				}
	...
	public List<TaskModel> taskDetails(TaskModel tm) {
		...
		StringBuilder sql = new StringBuilder();
		sql.append(
				" SELECT cmtd.task_detail_id, cmtd.task_id, cmtd.task_status, IFNULL(cmtd.actual_time,'-') AS actual_time, cmtd.actual_time_modified, ");
		// sql.append("
		// IFNULL(DATE_FORMAT(CONVERT_TZ(task_end_time,'"+UtilCollections.getServerTimeZone()+"','"+localoffsetTimeZone+"'),'%h:%i
		// %p'),'')AS 'endTime',");
		sql.append(" 	IFNULL ( DATE_FORMAT(CONVERT_TZ(cmtd.created_timestamp, '" + Utilcollection.getServerTimeZone()
				+ "', ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%b %d %Y'),'') AS 'startDate',");
		sql.append("  	IFNULL ( DATE_FORMAT(CONVERT_TZ(cmtd.created_timestamp,'" + Utilcollection.getServerTimeZone()
				+ "',  ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%h:%i %p'),'') AS 'startTime',");
		sql.append("  	IFNULL ( DATE_FORMAT(CONVERT_TZ(cmtd.created_timestamp,'" + Utilcollection.getServerTimeZone()
				+ "',  ifnull(cu.user_timezone, '" + localoffsetTimeZone + "') ),'%H:%i:%s'),'') AS 'startTime',");
	...
	public List<TaskModel> updateTaskValues(TaskModel tm) {
		...
		try {
			...
			if (updateType.equals("priority")) {
				...
				String query = "SELECT IFNULL(task_billable, 'N') as billableStatus FROM co_mykronus_tasks WHERE task_id="
						+ taskId;
			...
			} else if (updateType.equals("privacy")) {
				String taskPrivacy = tm.getTaskPrivacy();
				sql.append(" SET task_type='" + taskPrivacy + "'");
			}
			sql.append(" , last_updated_date= '" + Utilcollection.getDate() + "'");
			sql.append(" , last_updated_timestamp=  '" + Utilcollection.getTimeStamp() + "'");
	...
	public List<TaskModel> updateTaskActualHour(TaskModel tm) {
		...
		List<TaskModel> updateTaskActualHourDetails = new ArrayList<TaskModel>();
		String taskDetailId = (tm.getTask_detail_id() == null || tm.getTask_detail_id().equals("")) ? ""
				: tm.getTask_detail_id();
		String taskId = tm.getTask_id();
		String totActHour = tm.getToActHour();
		String actHourUpdateFlag = (tm.getActHourUpdateFlag() == null || tm.getActHourUpdateFlag().equals("")) ? "yes"
				: tm.getActHourUpdateFlag();
		String userId = tm.getUser_id();

		if (taskDetailId.equals("")) {
			// tm.setResult(result);
			// updateTaskActualHourDetails.add(tm);
			// return updateTaskActualHourDetails;
		...
		try {
			if (actHourUpdateFlag.equals("no")) {
				sql.append(" SELECT ");
				sql.append("  DATE_FORMAT(CONVERT_TZ(cmtd.task_status_stop_timestamp,'"
						+ Utilcollection.getServerTimeZone() + "', ifnull(cu.user_timezone, '" + localoffsetTimeZone
						+ "') ), '%Y-%m-%d') AS status_stop_date ");
				sql.append(" FROM co_mykronus_tasks_details cmtd ");
				sql.append(" JOIN co_users cu ON cu.user_id = cmtd.created_by ");
				sql.append(" WHERE cmtd.task_detail_id=").append(taskDetailId);
				String statusStopDate = jdbctm.queryForObject(sql.toString(), String.class);

				sql = new StringBuilder("");
				sql.append(" SELECT ");
				sql.append(
						" IFNULL(TIME_FORMAT(SEC_TO_TIME(SUM(IFNULL(TIME_TO_SEC(cuud.total_work_hours),0))) , '%H:%i'),  '-' )  AS TotalHrs ");
				sql.append(" FROM co_user_usage_details cuud ");
				sql.append(" JOIN co_users cu on cu.user_id = cuud.user_id ");
				sql.append(" WHERE cuud.user_id=" + userId
						+ " and DATE_FORMAT(CONVERT_TZ(cuud.user_usage_date_timestamp, '"
						+ Utilcollection.getServerTimeZone() + "', ifnull(cu.user_timezone, '" + localoffsetTimeZone
						+ "')), '%Y-%m-%d')  = '" + statusStopDate + "' ");
				String totalWorkHours = jdbctm.queryForObject(sql.toString(), String.class);
				sql = new StringBuilder("");
				sql.append(" SELECT IF(TIME_TO_SEC(time_format('" + totActHour
						+ "', '%H:%i') ) > TIME_TO_SEC(time_format('" + totalWorkHours
						+ "', '%H:%i') ) ,'Y','N') as ExceedStatus");
				String ExceedStatus = jdbctm.queryForObject(sql.toString(), String.class);
				if (ExceedStatus.equals("Y")) {
					result = "exceeded";
					tm.setResult(result);
					updateTaskActualHourDetails.add(tm);
					return updateTaskActualHourDetails;
			...
			if (updateFlag) {
				sql = new StringBuilder("");
				sql.append(" UPDATE co_mykronus_tasks_details  ");
				sql.append(" SET actual_time='" + totActHour + "' ");
				sql.append(" , actual_time_modified='Y' ");
				sql.append(" , last_updated_date= '" + Utilcollection.getDate() + "'");
				sql.append(" , last_updated_timestamp=  '" + Utilcollection.getTimeStamp() + "'");
				sql.append(" , last_updated_by=" + userId);
				sql.append("  WHERE task_detail_id=" + taskDetailId);
				jdbctm.update(sql.toString());

				sql = new StringBuilder("");
				sql.append(" UPDATE co_mykronus_tasks  ");
				sql.append(" SET actual_time_modified='Y' ");
				sql.append(" , last_updated_date= '" + Utilcollection.getDate() + "'");
				sql.append(" , last_updated_timestamp=  '" + Utilcollection.getTimeStamp() + "'");
				sql.append(" , last_updated_by=" + userId);
				sql.append("  WHERE task_id=" + taskId);
				jdbctm.update(sql.toString());
				updateTaskActualHourDetails = getTaskDetails(tm);
